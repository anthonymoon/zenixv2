...
# Remote installation script for NixOS ephemeral root ZFS
# Syncs repository to target and executes installation remotely
...
if [ $# -ne 3 ]; then
    ...
    echo ""
    echo "This will:"
    echo " - Rsync this repository to root@<target-ip>"
    echo " - SSH into the target and run the installer"
    echo " - Configure the system with the specified hostname and disk"
    exit 1
fi
...
echo "=== NixOS Remote Installation ==="
...
if [[ "$DISK" == /dev/vd* ]]; then
    ...
    ssh -t root@"$TARGET_IP" "cd /tmp/nixos-install && bash ./install-vm.sh '$HOSTNAME' '$DISK'"
else
    echo "Using optimized installer for physical disks..."
    ssh -t root@"$TARGET_IP" "cd /tmp/nixos-install && bash ./install-optimized.sh '$HOSTNAME' '$DISK'"
fi

echo ""
echo "Remote installation initiated!"
echo "The installer will handle:"
echo " - Disk partitioning with disko"
echo " - ZFS pool creation"
echo " - NixOS installation"
echo ""
echo "After installation completes:"
echo " - System will have hostname: $HOSTNAME"
echo " - SSH access: root:nixos, amoon:nixos, nixos:nixos"
echo " - Your SSH key is already authorized"
echo ""
echo "You can then rebuild remotely with:"
